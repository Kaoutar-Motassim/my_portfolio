...
export default function Orb({
  ...
  const frag = /* glsl */ `
    ...
    float light1(float intensity,float atten,float dist){
      return intensity/(1.0+dist*atten);
    }
    float light2(float intensity,float atten,float dist){
    ...
    vec4 draw(vec2 uv){
      ...
      float particle = smoothstep(0.02,0.0,length(fract(uv*10.0+iTime*0.1)-0.5));
      col += particle*vec3(0.8,0.8,1.0);
    ...
    vec4 mainImage(vec2 fragCoord){
      ...
      // Hover gravity effect
      uv.x += hover*hoverIntensity*0.1*sin(uv.y*10.0+iTime);
      uv.y += hover*hoverIntensity*0.1*cos(uv.x*10.0+iTime);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = e => {
      ...
      targetHover = Math.sqrt(uvX*uvX+uvY*uvY)<1.5 ? 1 : 0;
    };
    ...
    const update = t => {
      ...
      program.uniforms.hue.value = (hue+t*0.02)%360;
